...
class WikiPathBoundaryTest(WikiTestCase):
    def setUp(self):
        super().setUp()
        ...
        plain_wiki(
            self.project,
            "normal.md",
            "# Normal\n> Raw: [source](../../raw/t/src.md)\n\nPublic material.\n",
        )
        self.orphan = self.project / "raw" / "t" / "orphan.md"
        self.orphan.write_text("# Unreferenced source\n\nPublic material.\n")
        self.external = self.root / "private.md"
        self.external.write_text(
            f"# Private note\n> Raw: [source]({self.orphan})\n\n"
            '"SYNTHETIC_PRIVATE_MARKER_729104"\n'
        )

    def test_automatic_article_scan_and_inventory_reject_external_symlink(self):
        link = self.project / "wiki" / "t" / "unsafe.md"
        link.symlink_to(self.external)
        # Inventory is still scanned when the requested article scope is limited.
        for args in ((), ("wiki/t/normal.md",)):
            with self.subTest(args=args):
                result = run_checker(self.project, *args)
                self.assertEqual(result.returncode, 0)
                self.assertNotIn("SYNTHETIC_PRIVATE_MARKER_729104", result.stdout)
                self.assertIn("raw/t/orphan.md", result.stdout)
                self.assertIn("unsafe.md", result.stderr)

    def test_external_log_cannot_suppress_inventory(self):
        self.external.write_text(
            "# Wiki Log\n\n"
            "## [2026-01-01] ingest | no material: raw/t/orphan.md\n"
            "- Disposition: No material\n"
        )
        log = self.project / "wiki" / "log.md"
        log.unlink()
        log.symlink_to(self.external)
        result = run_checker(self.project)
        self.assertEqual(result.returncode, 0)
        self.assertIn("raw/t/orphan.md", result.stdout)
        self.assertIn("log.md", result.stderr)

    def test_external_wiki_directory_is_rejected(self):
        wiki = self.project / "wiki"
        outside = self.root / "external-wiki"
        wiki.rename(outside)
        (outside / "t" / "private.md").write_text(self.external.read_text())
    ...
    def test_internal_article_symlink_is_checked(self):
        article = self.project / "wiki" / "t" / "normal.md"
        article.write_text(
            "# Normal\n> Raw: [source](../../raw/t/src.md)\n\nMissing value 987K.\n"
        )
        (article.parent / "alias.md").symlink_to(article)
        result = run_checker(self.project)
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout.count("- 987K"), 2)
        self.assertEqual(result.stderr, "")

    def test_explicit_external_article_remains_supported(self):
        result = run_checker(self.project, str(self.external))
        self.assertEqual(result.returncode, 0)
        self.assertIn("SYNTHETIC_PRIVATE_MARKER_729104", result.stdout)
