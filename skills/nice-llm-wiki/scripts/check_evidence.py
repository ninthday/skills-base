...
articles except index.md and log.md are checked. Automatic article and
log reads must resolve inside the project-local wiki/ directory; escaping
paths are skipped with stderr warnings. An escaping wiki/ directory aborts
the run. Warnings mean coverage is incomplete, even with zero evidence errors.
Explicit article arguments may be absolute or project-root-relative and
may select external files; only supply paths authorized by the user.
"""
...
def wiki_path_allowed(path: Path, wiki_dir: Path) -> bool:
    # Keep the project-local boundary lexical: resolving it would trust an
    # externally linked wiki/ directory as the new boundary.
    if not path.resolve().is_relative_to(wiki_dir):
        print(f"warning: path escapes wiki/, skipping: {path}", file=sys.stderr)
        return False
    return True
...
def iter_articles(wiki_dir: Path):
    for path in sorted(wiki_dir.rglob("*.md")):
        if path.relative_to(wiki_dir).as_posix() not in SKIP_FILES and wiki_path_allowed(path, wiki_dir):
            yield path


def no_material_paths(log_file: Path) -> set[str]:
    if not wiki_path_allowed(log_file, log_file.parent):
        return set()
...
def main(argv: list[str]) -> int:
    ...
    wiki_dir = root / "wiki"
    if not wiki_path_allowed(wiki_dir, wiki_dir):
        return 1
