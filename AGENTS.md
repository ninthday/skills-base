...
- **Config:** Each vendor specifies which skills to sync and their output names in `meta.py`
...
```
.
├── meta.py                     # Project metadata (repos & URLs)
...
**Important:** For Type 1 (generated), the `skills/{project}/` name must match `sources/{project}/`. For Type 2 (synced), the output name is configured in `meta.py` and may differ from the source skill name.
...
1. **Add entry to `meta.py`** in the `submodules` mapping:
   ```python
   submodules = {
       # ... existing entries
